...
static GetNextCameraPosition(move_mode, Float:CP[3], Float:FV[3], &Float:X, &Float:Y, &Float:Z)
{
    // Calculate the cameras next position based on their current position and the direction their camera is facing
...
static CancelFlyMode(playerid)
{
	...
	noclipdata[playerid][cameramode] = CAMERA_MODE_NONE;
	SetPlayerPos(playerid, pPos[0], pPos[1], pPos[2]+1.5);
	return 1;
}

//--------------------------------------------------

static FlyMode(playerid)
{
